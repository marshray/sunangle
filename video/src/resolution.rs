...
pub static VIDEO_RESOLUTION_720: Lazy<VideoResolution> = Lazy::new(|| VideoResolution {
    names: ["720", "1280x720", "HD", "WXGA"].iter().map(|&s| s.into()).collect(),
    dims_px: [1280, 720],
    pixel_aspect: Ratio::new_raw(1, 1),
});

pub static VIDEO_RESOLUTION_1080: Lazy<VideoResolution> = Lazy::new(|| VideoResolution {
    names: ["1080", "1920x1080", "Full HD"].iter().map(|&s| s.into()).collect(),
    dims_px: [1920, 1080],
    pixel_aspect: Ratio::new_raw(1, 1),
});

pub static COMMMON_VIDEO_RESOLUTIONS: Lazy<Vec<&VideoResolution>> = Lazy::new(|| vec![
